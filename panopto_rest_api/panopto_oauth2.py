...
class PanoptoOAuth2():
    def __init__(self, server, client_id, client_secret, ssl_verify):
        ...
        # Create URI from
        self.authorization_endpoint = 'https://{0}/Panopto/oauth2/connect/authorize'.format(
            server)
        self.access_token_endpoint = 'https://{0}/Panopto/oauth2/connect/token'.format(
            server)
    ...
    def get_access_token_authorization_code_grant(self):
        ...
        print()
        print('Opening the browser for authorization: {0}'.format(
            authorization_url))
        ...
        with RedirectTCPServer() as httpd:
            print('HTTP server started at port {0}. Waiting for redirect.'.format(
                REDIRECT_PORT))
            # Serve one request.
...
class RedirectHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(
            '<html><body><p>Authorization redirect was received. You may close this page.</p></body></html>'.encode('utf-8'))
        self.wfile.flush()
